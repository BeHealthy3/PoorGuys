...
struct PostView: View {
    ...
    var body: some View {
        ZStack {
            DivergeView(if: post.isWeirdPost, true: weirdPostView(), false: postView())
        }
    }
    
    @ViewBuilder
    private func weirdPostView() -> some View {
        Color.white
                ...
                HStack() {
                    ...
                    HStack(spacing: 8) {
                        HStack(spacing: 2) {
                            ...
                            Text(String(post.comments.filter { $0.isDeletedComment == false }.count))
